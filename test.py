...
# load 3d object file
object_indices, object_vertices = ObjectLoader.load_obj('assets/3dObject/castle/castle.obj')
...
# load image
exterior_image = Image.open("assets/3dObject/castle/Castle Exterior Texture.jpg")
exterior_image = exterior_image.transpose(Image.FLIP_TOP_BOTTOM)
exterior_img_data = exterior_image.convert("RGBA").tobytes()
glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, exterior_image.width, exterior_image.height, 0, GL_RGBA, GL_UNSIGNED_BYTE, exterior_img_data)

interior_image = Image.open("assets/3dObject/castle/Castle Interior Texture.jpg")
interior_image = interior_image.transpose(Image.FLIP_TOP_BOTTOM)
interior_img_data = interior_image.convert("RGBA").tobytes()
glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, interior_image.width, interior_image.height, 0, GL_RGBA, GL_UNSIGNED_BYTE, interior_img_data)
...
while not glfw.window_should_close(window):
    ...
    rotation_y_90 = pyrr.Matrix44.from_y_rotation(np.pi / 1.5)
